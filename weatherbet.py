...
Fixes applied vs original:
  1. bucket_prob() — was returning 1.0/0.0 binary for normal buckets;
     now returns the correct normal-distribution area between t_low and t_high.
     This was causing wildly overstated EV on every trade.
  2. Stop sizing — was using a flat 20% stop regardless of entry price.
     Low-priced contracts ($0.10–0.30) were getting stopped out by normal
     bid-ask noise. Stop is now adaptive: deeper for low-priced contracts.
  3. Re-entry cooldown — after a stop-loss the bot was immediately re-entering
     the same market on the next scan (Seoul loop). Added a 12-hour cooldown
     per city+date after any stop-loss exit.
  4. Sentinel value guard — parse_temp_range can return (-999, x) or (x, 999)
     for edge buckets. These now log a warning and are skipped in BUY logic
     rather than trading on them.
"""
...
# FIX 1: bucket_prob now uses normal distribution area for all bucket types.
# Original code returned 1.0 or 0.0 for normal buckets (binary), which made
# p=1.0 whenever forecast landed in a bucket and caused massive EV overstatement.
#
# Correct approach: probability = area under N(forecast, sigma) between t_low and t_high.
# For a 1°F bucket with sigma=2°F, this gives ~0.38 instead of 1.0.
# For a 2°F bucket it gives ~0.68. Much more realistic.
#
# Edge buckets (t_low=-999 or t_high=999) use the same formula naturally —
# they just integrate to -inf or +inf on one side.
def bucket_prob(forecast, t_low, t_high, sigma=None):
    """
    Probability that actual temp falls in [t_low, t_high] given forecast
    and uncertainty sigma, modelled as a normal distribution.
    """
    s = sigma or 2.0
    f = float(forecast)
    if t_low == -999:
        # "X or below" bucket — left tail
        return norm_cdf((t_high - f) / s)
    if t_high == 999:
        # "X or higher" bucket — right tail
        return 1.0 - norm_cdf((t_low - f) / s)
    # Normal bucket — area between t_low and t_high
...
# FIX 2: adaptive stop loss based on entry price.
# Original used a flat 20% stop regardless of entry price.
# On a $0.13 contract, 20% = $0.026 — well within normal bid-ask noise,
# so positions were getting stopped out immediately.
#
# New tiers:
#   entry < 0.15  →  40% stop  (very cheap contracts, need room to breathe)
#   entry < 0.25  →  35% stop
#   entry < 0.35  →  30% stop
#   entry >= 0.35 →  25% stop  (closer to market price, tighter is fine)
def calc_stop_price(entry_price):
    """Returns the stop-loss price for a given entry price."""
    if entry_price < 0.15:
        pct = 0.40
...
# FIX 4: guard against sentinel values leaking into buy logic.
def is_sentinel_range(t_low, t_high):
    """Returns True if either bound is a sentinel (-999 or 999)."""
    return t_low == -999.0 or t_high == 999.0
...
def new_market(city_slug, date_str, event, hours):
    ...
    return {
        ...
        "pnl":                None,
        "last_stop_at":       None,   # FIX 3: track when last stop fired
        "forecast_snapshots": [],
        "market_snapshots":   [],
        "all_outcomes":       [],
        ...
        for i, date in enumerate(dates):
            ...
            if mkt.get("position") and mkt["position"].get("status") == "open":
                ...
                if current_price is not None:
                    ...
                    # FIX 2: use adaptive stop, fall back to stored stop_price if set
                    stop = pos.get("stop_price") or calc_stop_price(entry)
                    if "stop_price" not in pos:
                        pos["stop_price"] = stop

                    # Trailing: if up 20%+ — move stop to breakeven
                    if current_price >= entry * 1.20 and pos["stop_price"] < entry:
                    ...
                    if current_price <= pos["stop_price"]:
                        ...
                        pos["pnl"]          = pnl
                        pos["status"]       = "closed"
                        # FIX 3: record when stop fired so we can enforce cooldown
                        mkt["last_stop_at"] = snap.get("ts")
                        closed += 1
                        reason = "STOP" if current_price < entry else "TRAILING BE"
                        print(f"  [{reason}] {loc['name']} {date} | entry ${entry:.3f} exit ${current_price:.3f} | {hours:.0f}h left | PnL: {'+'if pnl>=0 else ''}{pnl:.2f}")

            # --- CLOSE if forecast shifted outside bucket ---
            ...
            if not mkt.get("position") and forecast_temp is not None and hours >= MIN_HOURS:
                ...
                # FIX 3: enforce cooldown after a stop-loss on this market
                last_stop_at = mkt.get("last_stop_at")
                if last_stop_at:
                    try:
                        last_stop_dt = datetime.fromisoformat(last_stop_at.replace("Z", "+00:00"))
                        hours_since_stop = (now - last_stop_dt).total_seconds() / 3600
                        if hours_since_stop < STOP_COOLDOWN_HOURS:
                            remaining = STOP_COOLDOWN_HOURS - hours_since_stop
                            # Silently skip — no log spam, just don't re-enter
                            save_market(mkt)
                            continue
                    except Exception:
                        pass
                ...
                for o in outcomes:
                    ...
                    # FIX 4: skip sentinel edge buckets for BUY signals.
                    # We only want to bet on a specific temp range, not "anything above X".
                    # Edge buckets have highly uncertain probabilities and the sentinel
                    # values (-999, 999) were causing display/logic bugs downstream.
                    if is_sentinel_range(t_low, t_high):
                        continue
                ...
                if best_signal:
                    ...
                    if not skip_position and best_signal["entry_price"] < MAX_PRICE:
                        # Set adaptive stop price at position open time
                        best_signal["stop_price"] = calc_stop_price(best_signal["entry_price"])
...
def monitor_positions():
    ...
    for mkt in open_pos:
        ...
        entry = pos["entry_price"]
        # FIX 2: use stored adaptive stop, or calculate if missing (old positions)
        stop  = pos.get("stop_price") or calc_stop_price(entry)
        if "stop_price" not in pos:
            pos["stop_price"] = stop
        ...
        stop_triggered = current_price <= pos["stop_price"]
        ...
        if take_triggered or stop_triggered:
            ...
            if take_triggered:
                ...
                reason = "STOP"
                # FIX 3: record stop time for cooldown
...
def run_loop():
    ...
    print(f"  API:        http://0.0.0.0:8080/logs  |  /status  |  /health")
