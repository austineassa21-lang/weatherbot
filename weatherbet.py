...
API:
    uvicorn weatherbet:app --host 0.0.0.0 --port 8080   # start API server
    GET /logs       # last 200 log lines (plain text)
    GET /status     # balance and open positions (JSON)
...
logging.basicConfig(level=logging.INFO, handlers=[_buf_handler, _stdout_handler])
log = logging.getLogger("weatherbet")

# Replace print() calls with log.info() so they appear in /logs
_real_print = print
...
def get_status():
    ...
    return {
        "balance":      bal,
        "start":        start,
        "return_pct":   round((bal - start) / start * 100, 2),
        "trades":       total,
        "wins":         wins,
        "losses":       losses,
        ...
        "peak_balance": state.get("peak_balance", bal),
    }
...
if __name__ == "__main__":
    ...
    if cmd == "run":
        # Start API server in background, then run the bot loop
        ...
        # API-only mode (for uvicorn direct launch)
        _cal = load_cal()
