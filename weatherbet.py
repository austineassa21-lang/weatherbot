...
Fixes vs original:
  1. bucket_prob() — was binary (1.0/0.0); now uses normal distribution area.
  2. Price parsing — outcomePrices[0]/[1] are YES/NO prices, NOT bid/ask.
     Real bid/ask come from bestBid/bestAsk fields on each market object.
  3. parse_temp_range() — unicode degree symbol (°) and format variants fixed.
  4. Stop sizing — adaptive based on entry price, not flat 20%.
  5. Re-entry cooldown — 12h cooldown after stop-loss on same market.
  6. Sentinel value guard — edge buckets (-999/999) skipped for BUY signals.
"""
...
MIN_VOLUME           = _cfg.get("min_volume", 200)
...
def calc_stop_price(entry_price):
    """Adaptive stop loss — deeper for cheap contracts to survive noise."""
    if entry_price < 0.15:
        pct = 0.40
...
def parse_temp_range(question):
    ...
    Handles formats observed in live API:
      "...be 45°F or below..."          -> (-999, 45)
      "...be 64°F or higher..."         -> (64, 999)
      "...be between 48-49°F..."        -> (48, 49)
      "...be between 48-49°C..."        -> (48, 49)

    The degree symbol is unicode U+00B0 (°). Regex uses a character
    class [\u00b0°]? to match it optionally since some questions may omit it.
    """
    if not question:
        return None

    num = r'(-?\d+(?:\.\d+)?)'
    deg = r'[\u00b0°]?'   # optional degree symbol, handles unicode °

    # "45°F or below" / "45°C or below"
    m = re.search(num + deg + r'[FC]\s+or\s+below', question, re.IGNORECASE)
    if m:
        return (-999.0, float(m.group(1)))

    # "64°F or higher" / "64°C or higher"
    m = re.search(num + deg + r'[FC]\s+or\s+higher', question, re.IGNORECASE)
    if m:
        return (float(m.group(1)), 999.0)

    # "between 48-49°F" / "between 48-49°C"
    m = re.search(r'between\s+' + num + r'\s*-\s*' + num + deg + r'[FC]', question, re.IGNORECASE)
    if m:
        return (float(m.group(1)), float(m.group(2)))

    # "be 52°F on" — exact single value
...
def parse_market_prices(market):
    """
    Extract bid and ask from a Polymarket market object.

    The outcomePrices field is ["yes_price", "no_price"] — NOT bid/ask.
    Real bid/ask are in bestBid and bestAsk fields directly.
    Falls back to yes_price from outcomePrices if bestBid/bestAsk missing.
    """
    ...
    # Fallback: use yes outcomePrices as approximate mid price
    try:
...
def scan_and_update():
    ...
    for city_slug, loc in LOCATIONS.items():
        ...
        for i, date in enumerate(dates):
            ...
            # Build outcomes — use bestBid/bestAsk directly from each market object
            outcomes = []
            ...
            if not mkt.get("position") and forecast_temp is not None and hours >= MIN_HOURS:
                ...
                if last_stop_at:
                    try:
                        ...
                        if (now - last_stop_dt).total_seconds() / 3600 < STOP_COOLDOWN_HOURS:
                            save_market(mkt)
                            continue
                    except Exception:
                        pass
                ...
                # Evaluate ALL non-sentinel buckets and pick the one with the
                # highest EV. This is the core change from the original logic
                # which only checked the bucket the forecast landed in.
                #
                # Why this matters: with sigma=2°F and 2°F-wide buckets, the
                # forecast bucket peaks at p≈0.38. Adjacent buckets get p≈0.22
                # but are often priced at $0.10-0.18 — much better EV. We were
                # leaving those on the table entirely.
                #
                # We still require the bucket to be within 2 sigma of the
                # forecast to avoid betting on completely implausible outcomes.
                best_signal = None
                best_ev     = MIN_EV  # only beat this to become best_signal
                ...
                for o in outcomes:
                    ...
                    # Skip buckets more than 2 sigma away from forecast —
                    # probability will be negligible and likely just noise
                    bucket_mid = (t_low + t_high) / 2
                    if abs(bucket_mid - forecast_temp) > 2 * sigma * 2:
                        continue
                ...
                if best_signal:
                    # Re-fetch real bestAsk/bestBid to confirm before committing
                    skip_position = False
                    try:
                        r = requests.get(
                        ...
                        mdata      = r.json()
                        real_ask   = float(mdata.get("bestAsk") or best_signal["entry_price"])
                        real_bid   = float(mdata.get("bestBid") or best_signal["bid_at_entry"])
...
def monitor_positions():
    ...
    for mkt in open_pos:
        ...
        take_profit = None if hours_left < 24 else (0.85 if hours_left < 48 else 0.75)
    ...
    if open_pos:
        ...
        for m in open_pos:
            ...
            print(f"    {m['city_name']:<16} {m['date']} | {label:<14} | "
                  ...
                  f"PnL: {'+'if unrealized>=0 else ''}{unrealized:.2f} | {pos['forecast_src'].upper()}")
        print(f"\n  Unrealized PnL: {'+'if total_unrealized>=0 else ''}{total_unrealized:.2f}")
    print(f"{'='*55}\n")
