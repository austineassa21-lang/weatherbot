...
config.json keys:
    ...
    min_ev           minimum edge to trade, e.g. 0.10 = 10% (default 0.10)
...
MIN_HOURS           = _cfg.get("min_hours", 24)       # skip D+0 by default
...
def fetch_ensemble(city_slug, dates):
    ...
    if loc["region"] == "us":
        ...
        for d in dates:
            ...
            # Weight GFS at 0.6x relative to ECMWF for blending —
            # GFS is better short-range for US, ECMWF better medium-range.
            # We add them as extra samples rather than averaging to preserve
            # the full distribution shape.
            results[d].extend(gfs)

    # --- Fallback: single forecast + synthetic spread ---
    # If ensemble API returns nothing (quota, network), fall back to the
    # regular forecast API and synthesise a spread using historical MAE.
    for d in dates:
        if not results[d]:
            ...
            if fallback is not None:
                # Synthesise 50 members by adding Gaussian noise using
                # a default sigma of 2°F / 1.2°C as uncertainty estimate
...
def _fetch_ensemble_model(lat, lon, model, temp_unit, tz, dates):
    ...
    for attempt in range(3):
        try:
            ...
            # Member keys look like "temperature_2m_max_member00" etc.
            member_keys = [k for k in daily if k.startswith("temperature_2m_max_member")]
            if not member_keys:
                # Some models return a single "temperature_2m_max" key
...
def ensemble_stats(members):
    ...
    if std < 1.5:
        spread = "tight"      # high confidence
    elif std < 3.0:
        spread = "moderate"
    elif std < 5.0:
        spread = "wide"
    else:
        spread = "very_wide"  # low confidence, be cautious
...
def mc_bucket_probs(members, buckets, n_sims=None):
    ...
    Why Monte Carlo instead of just counting members directly:
      - The ensemble has only 50 members. With 2000 MC draws we smooth
        out sampling noise and get stable probability estimates.
      - We add a small residual uncertainty (sigma_res) to account for
        model error not captured by ensemble spread — the ensemble is
        often overconfident (under-dispersed).
      - This gives better calibration on thin buckets.

    Args:
        members: list of ensemble temperature values
        buckets: list of (t_low, t_high) tuples
        n_sims:  number of Monte Carlo samples to draw

    Returns:
        dict mapping (t_low, t_high) -> probability
    """
    ...
    # Residual uncertainty: ensemble spread is typically under-dispersed
    # by about 20-30%. We add a small fixed residual to compensate.
    # Values from empirical calibration studies on NWP ensembles.
    # 0.8°F for tight, scaling up for wider spreads.
    spread_map = {"tight": 0.8, "moderate": 1.2, "wide": 1.8, "very_wide": 2.5, "unknown": 1.5}
    sigma_res  = spread_map.get(stats["spread"], 1.5)

    # Draw samples: each draw = pick a random member + add residual noise
...
def find_best_signal(members, outcomes, unit, min_price=None, max_price=None,
    ...
    # Only evaluate buckets within reasonable range of the ensemble mean.
    # Skip anything more than 3 std away — probability will be negligible
    # and we'd just be chasing noise.
    mean  = stats["mean"]
    std   = max(stats["std"] or 2.0, 1.0)   # floor at 1° to avoid division issues
    window = 3.0 * std

    # Build list of (t_low, t_high) for all non-sentinel, in-window buckets
    candidate_buckets = []
    outcome_map       = {}
    for o in outcomes:
        t_low, t_high = o["range"]
        # Skip sentinel edge buckets — hard to price accurately
    ...
    # Run Monte Carlo to get probability per bucket
    probs = mc_bucket_probs(members, candidate_buckets)

    # Find highest-EV bucket
    best_ev     = min_ev
    best_signal = None
    ...
    if best_signal:
        # Sanity check: don't bet on near-certainties or near-impossibilities.
        # If p > 0.80 the market should be pricing it much higher — something
        # is wrong with our ensemble or the market data.
        # If p < 0.03 it's too unlikely to be worth the transaction cost.
        p = best_signal["p"]
        if p > 0.80 or p < 0.03:
            return None
...
def calc_stop_price(entry_price):
    ...
    """
    if entry_price < 0.10:
        return None   # too cheap — don't trade (caught earlier by MIN_PRICE)
...
def parse_market_prices(market):
    ...
    # Fallback to yes price as approximate mid
    try:
...
def scan_and_update(simulate=False):
    ...
    for city_slug, loc in LOCATIONS.items():
        ...
        # D+1 through D+3 only — skip today (D+0) to avoid stale-data trades
        dates = [(now + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(1, 4)]

        # Fetch ensemble for all dates in one API call
        try:
        ...
        for i, date in enumerate(dates):
            ...
            # ----------------------------------------------------------------
            # STOP-LOSS / TRAILING STOP (monitor open position)
            # ----------------------------------------------------------------
            if mkt.get("position") and mkt["position"].get("status") == "open":
                ...
                if current_price is not None:
                    ...
                    # Move stop to breakeven if up 25%+
            ...
            # Close position if ensemble mean has moved significantly away
            # from the bet bucket — the thesis has changed.
            # ----------------------------------------------------------------
            if mkt.get("position") and mkt["position"].get("status") == "open" and members:
                ...
                if mean is not None:
                    ...
                    # Close if mean has drifted more than 1.5 std from bucket center
            ...
            if not mkt.get("position") and members:
                ...
                # Cooldown after stop-loss on this market
                ...
                # Skip if ensemble spread is very wide — we have no edge
                ...
                if signal:
                    ...
                    # Re-check filters with live prices
                    ...
                    print(f"  {tag}[BUY]  {loc['name']} {horizon} {date} | {bucket_label} | "
                          f"${real_ask:.3f} | p={signal['p']:.3f} | EV {live_ev:+.2f} | "
                          f"${size:.2f} | mean={signal['ensemble_mean']:.1f}±{signal['ensemble_std']:.1f}{unit_sym} "
    ...
    # ------------------------------------------------------------------------
    # AUTO-RESOLUTION: check if any open positions have resolved on Polymarket
    # ------------------------------------------------------------------------
    if not simulate:
...
def monitor_positions():
    ...
    for mkt in open_pos:
        ...
        # Trailing stop: move to breakeven if up 25%+
        if current_price >= entry * 1.25 and pos.get("stop_price", 0) < entry:
            pos["stop_price"] = entry
            pos["trailing_activated"] = True
            print(f"  [TRAIL] {city_name} {mkt['date']} — stop → breakeven ${entry:.3f}")

        # Take profit: lock in gains closer to resolution
        take_profit = None
        if hours_left < 24:
...
def print_report():
    ...
    # Calibration: compare bet probability to actual win rate by p-bucket
    ...
    for lo, hi in p_buckets:
        ...
        print(f"    p={lo:.2f}-{hi:.2f}: n={len(group)} | avg bet p={avg_p:.3f} | actual WR={actual_wr:.0%}")
    ...
    for m in sorted(resolved, key=lambda x: x["date"]):
        ...
        print(f"    {m['city_name']:<16} {m['date']} | {label:<12} | {p_str} | "
